...
class RobotFusion(object):
  ...
  def __init__(self, PI_s, PI_t, alpha=0.5, Sm=1.5):
    ...
    self.point_pub = rospy.Publisher(
        '/estimated_pose', PointStamped, queue_size=6)
    self.colud_point_pub = rospy.Publisher(
        '/transformed_particles', PointCloud, queue_size=20)
  ...
  def chernoff_fusion(self, x, PF_1, PF_2, alpha, beta):
    return ((np.sum(-beta * np.linalg.norm(PF_1[:, 0:2] - x)**2))**alpha) * (
        (np.sum(-beta * np.linalg.norm(PF_2[:, 0:2] - x)**2))**(1 - alpha))

  def update_weight(self):
    W = np.empty(len(self.PI_s))
    for ii in range(len(self.PI_s)):
      W[ii] = self.chernoff_fusion(self.PI_s[ii, 0:2], self.PI_s, self.PI_t,
                                   self.alpha, 8)
    W = W / np.sum(W)
...
def particles2fuse(msg, pf,
                   fusion):  # callback function from topic /particlecloud
  global particles
  ...
  for i in range(len(msg.poses)):
    ...
    angle = (msg.poses[i].orientation.x, msg.poses[i].orientation.y,
             msg.poses[i].orientation.z, msg.poses[i].orientation.w)
    particles[i, 2] = tf_conversions.transformations.euler_from_quaternion(
        angle)[2]
