...
class ParticleFilter(object):
    ...
    def __init__(self, Np=100):
        ...
        self.pub_estimated_pos = rospy.Publisher(
            '/MCL_estimated_pose', PoseWithCovarianceStamped, queue_size=60)
    ...
    def prediction(self):  # Odometry = Odometry massege. donot forget to initialize self.last_time =
        ...
        delta[:, 2] = dot[:, 2] * self.dt + \
            sigma_theta * np.random.randn(self.Np)
    ...
    def likelihood_fild(self):
        ...
        for ii in range(self.Np):
            ...
            self.weights[ii] = self.weights[ii] * \
                np.prod(np.exp(-(1.1) * np.linalg.norm(z_star-z, axis=1)**2))
