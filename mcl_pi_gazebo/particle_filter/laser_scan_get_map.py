...
class MapClientLaserScanSubscriber(object):
  ...
  def scan2cart(self, robot_origin=[0, 0, 0]):
    ...
    n = len(self.z.ranges)
    i = np.arange(len(self.z.ranges))
    angle = np.zeros(n)
    x = np.zeros(n)
    y = np.zeros(n)

    angle = np.add(self.z.angle_min, np.multiply(
        i, self.z.angle_increment)) + theta
  ...
  def obs(self):
    ...
    ) * self.map.info.resolution + self.map.info.resolution * 0.5 + self.map.info.origin.position.x

  def loction_based(self, y=[0, 0, 0]):
    Y = np.array(
        [y[0] + self.scan2cart(y)[0, :], y[1] + self.scan2cart(y)[1, :]])
    map_info = self.map.info
    map_width = np.array(map_info.width)  # map width
    map_heghit = np.array(map_info.height)  # map heghit
