...
class MapClientLaserScanSubscriber(object):
    ...
    def get_map(self, msg):     # callback function upon receiving occupancy map msg
        self.map = msg
        map_info = self.map.info
        map_width = np.array(map_info.width) # map width
        map_heghit = np.array(map_info.height) # map heghit
        self.occupancy_grid = np.transpose(np.array(self.map.data).reshape(map_width, map_heghit)) # map
