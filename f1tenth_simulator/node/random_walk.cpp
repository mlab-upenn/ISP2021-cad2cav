...
public:
  ...
  void odom_callback(const nav_msgs::Odometry&) {
    // publishing is done in odom callback just so it's at the same rate as the
    // sim
    ...
    // compute random amount to change desired angle by (between -range and
    // range)
    double rand_ang = range * random - range / 2.0;

    // sometimes change sign so it turns more (basically add bias to continue
    // turning in current direction)
    ...
    drive_msg.steering_angle =
        std::min(std::max(prev_angle + rand_ang, -max_steering_angle),
                 max_steering_angle);
