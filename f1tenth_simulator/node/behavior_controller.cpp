...
public:
  BehaviorController() {
    ...
    // get topic names
    std::string scan_topic, odom_topic, imu_topic, joy_topic, keyboard_topic,
        brake_bool_topic, mux_topic;
    ...
    laser_sub =
        n.subscribe(scan_topic, 1, &BehaviorController::laser_callback, this);
    joy_sub =
        n.subscribe(joy_topic, 1, &BehaviorController::joy_callback, this);
    imu_sub =
        n.subscribe(imu_topic, 1, &BehaviorController::imu_callback, this);
    odom_sub =
        n.subscribe(odom_topic, 1, &BehaviorController::odom_callback, this);
    key_sub =
        n.subscribe(keyboard_topic, 1, &BehaviorController::key_callback, this);
    ...
    collision_file.open(ros::package::getPath("f1tenth_simulator") + "/logs/" +
                        filename + ".txt");
    ...
    if (state.velocity != 0) {
      for (size_t i = 0; i < msg.ranges.size(); i++) {
        ...
        if ((ttc < ttc_threshold) && (ttc >= 0.0)) {
          ...
          collision_file << "Time since start of sim: "
      ...
      // if it's gone through all beams without detecting a collision, reset
      // in_collision
      in_collision = false;
    }
  }
  ...
  void imu_callback(const sensor_msgs::Imu&) {}
};
