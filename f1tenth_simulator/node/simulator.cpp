...
public:
  RacecarSimulator() : im_server("racecar_sim") {
    ...
    drive_sub =
        n.subscribe(drive_topic, 1, &RacecarSimulator::drive_callback, this);
    ...
    pose_rviz_sub = n.subscribe(pose_rviz_topic, 1,
                                &RacecarSimulator::pose_rviz_callback, this);

    // obstacle subscriber
    obs_sub =
        n.subscribe("/clicked_point", 1, &RacecarSimulator::obs_callback, this);
    ...
    // origin_x+(1/3)*map_width*map_resolution; clear_obs_button.pose.position.y
  ...
  void update_pose(const ros::TimerEvent &) {
    ...
    state.steer_angle = std::min(
        std::max(state.steer_angle, -max_steering_angle), max_steering_angle);
  ...
  void set_steer_angle_vel(double steer_angle_vel_) {
    steer_angle_vel = std::min(std::max(steer_angle_vel_, -max_steering_vel),
                               max_steering_vel);
  }
