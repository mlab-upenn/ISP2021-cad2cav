...
ScanSimulator2D::ScanSimulator2D(int num_beams_, double field_of_view_,
...
void ScanSimulator2D::scan(const Pose2D& pose, double* scan_data) {
  // Make theta discrete by mapping the range [-pi,pi] onto [0,
  ...
  for (int i = 0; i < num_beams; i++) {
    ...
    if (scan_std_dev > 0) scan_data[i] += noise_dist(noise_generator);
...
double ScanSimulator2D::trace_ray(double x, double y,
                                  double theta_index) const {
...
void ScanSimulator2D::set_map(const std::vector<double>& map, size_t height_,
                              size_t width_, double resolution_,
                              const Pose2D& origin_, double free_threshold) {
  // Assign parameters
  height     = height_;
  width      = width_;
...
void ScanSimulator2D::set_map(const std::vector<double>& map,
...
void ScanSimulator2D::xy_to_row_col(double x, double y, int* row,
                                    int* col) const {
  ...
  if (x_rot < 0 or x_rot >= width * resolution or y_rot < 0 or
