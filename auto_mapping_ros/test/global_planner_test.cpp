...
static constexpr auto csv_filepath =
    "/home/yash/yasht_ws/src/auto_mapping_ros/csv/sequence.csv";

int main(int argc, char** argv) {
  const auto csv_filepath =
      ros::package::getPath("auto_mapping_ros") + "/csv/sequence.csv";

  ros::init(argc, argv, "global_planner");

  auto nh = std::make_shared<ros::NodeHandle>(ros::NodeHandle());
  ros::Publisher vis_pub =
      nh->advertise<visualization_msgs::MarkerArray>("coverage_points", 0);
