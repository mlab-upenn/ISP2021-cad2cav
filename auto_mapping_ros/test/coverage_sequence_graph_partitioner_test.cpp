...
std::pair<Graph, cv::Mat> get_f110_map_graph() {
  const auto filepath =
      ros::package::getPath("auto_mapping_ros") + "/maps/levine_4.jpg";
  const auto csv_filepath =
      ros::package::getPath("auto_mapping_ros") + "/csv/sequence";
...
int main(int argc, char const* argv[]) {
  ...
  ROS_WARN(
      "Visualizing graph node IDs. Sequence 0 --> 1 goes from light to dark.");
  std::vector<std::array<int, 2>> sequence;
  for (int i = 0; i < gp_graph.size(); ++i) {
    sequence.emplace_back(
        std::array<int, 2>{static_cast<int>(gp_graph.getNode(i).y),
