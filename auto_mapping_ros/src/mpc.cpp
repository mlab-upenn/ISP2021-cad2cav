...
MPC::MPC(ros::NodeHandle &nh)
    : input_size_(2), state_size_(3), constraints_(nh), nh_(nh) {
  ...
  // dynamics + follow the gap + max/min input + skid limit
  num_constraints_ =
      (num_states_ + 2 * (horizon_ + 1) + num_inputs_ + 2 * horizon_);
...
MPC::~MPC() { ROS_INFO("killing the mpc"); }

float MPC::dt() { return dt_; }

int MPC::horizon() { return horizon_; }

std::vector<Input> MPC::solved_trajectory() { return solved_trajectory_; }

void MPC::UpdateScan(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
  scan_msg_ = *scan_msg;
}

void MPC::Update(State current_state, Input input,
                 std::vector<State> &desired_state_trajectory) {
...
void MPC::Visualize() {
  ...
  for (int ii = 0; ii < horizon_; ++ii) {
    ...
    predicted_input.set_steer_ang(
        full_solution_(num_states_ + ii * input_size_ + 1));
  ...
  //     point.x = desired_state_trajectory_[ii].x() + 0.4 *
  //     cos(desired_state_trajectory_[ii].ori()); point.y =
  //     desired_state_trajectory_[ii].y() + 0.4 *
  //     sin(desired_state_trajectory_[ii].ori()); point.z = 0.2;
  //     points_.push_back(point);
  ...
  // mpc_pub_.publish(Visualizer::GenerateList(points_, colors_,
...
void MPC::CreateHessianMatrix() {
  ...
  for (int ii = 0; ii < horizon_; ++ii) {  // change for terminal cost
    SparseBlockInit(hessian_, cost_.r(), num_states_ + ii * input_size_,
                    num_states_ + ii * input_size_);
  }
}

void MPC::CreateGradientVector() {
  for (int ii = 0; ii < horizon_; ++ii) {
    gradient_.block(ii * state_size_, 0, state_size_, 1) =
        -1 * cost_.q() * desired_state_trajectory_[ii].ToVector();
    gradient_.block(num_states_ + ii * input_size_, 0, input_size_, 1) =
        -1 * cost_.r() * desired_input_.ToVector();
  }
  gradient_.block(horizon_ * state_size_, 0, 3, 1) =
      -1 * cost_.q() * desired_state_trajectory_[horizon_ - 1].ToVector();
}
...
void MPC::CreateLinearConstraintMatrix() {
  ...
  a_eye << Eigen::MatrixXd::Identity(state_size_, state_size_),
  ...
  for (int ii = 1; ii < horizon_ + 1; ++ii) {
    SparseBlockInit(linear_matrix_,
                    Eigen::MatrixXd::Identity(state_size_, state_size_),
                    ii * state_size_, (ii - 1) * state_size_);
    SparseBlockInit(linear_matrix_,
                    Eigen::MatrixXd::Identity(state_size_, input_size_),
                    ii * state_size_, num_states_ + (ii - 1) * input_size_);
    SparseBlockInit(linear_matrix_, gap_con, num_states_ + 2 * (ii),
                    (ii)*state_size_);
    SparseBlockInit(linear_matrix_, constraints_.slip_constraint(),
                    num_states_ + 2 * (horizon_ + 1) + num_inputs_ + ii - 1,
                    num_states_ + (ii - 1) * input_size_);
  }

  SparseBlockEye(linear_matrix_, num_inputs_, num_states_ + 2 * (horizon_ + 1),
                 num_states_, 1);
}
...
void MPC::UpdateLinearConstraintMatrix() {
  ...
  gap_con(1, 2) = 0;
  for (int ii = 1; ii < horizon_ + 1; ++ii) {
    SparseBlockSet(linear_matrix_, model_.a(), ii * state_size_,
                   (ii - 1) * state_size_);
    SparseBlockSet(linear_matrix_, model_.b(), ii * state_size_,
                   num_states_ + (ii - 1) * input_size_);
    SparseBlockSet(linear_matrix_, gap_con, num_states_ + 2 * (ii),
                   (ii)*state_size_);
  }
}
...
void MPC::CreateLowerBound() {
  ...
  lower_bound_ << Eigen::VectorXd::Zero((horizon_ + 1) * state_size_),
      gap_con.replicate(horizon_ + 1, 1),
      constraints_.u_min().replicate(horizon_, 1),
      constraints_.slip_lower_bound().replicate(horizon_, 1);
}
...
void MPC::CreateUpperBound() {
  ...
  gap_con(0) = OsqpEigen::INFTY;
  gap_con(1) = OsqpEigen::INFTY;
  upper_bound_ << Eigen::VectorXd::Zero((horizon_ + 1) * state_size_),
      gap_con.replicate(horizon_ + 1, 1),
      constraints_.u_max().replicate(horizon_, 1),
      constraints_.slip_upper_bound().replicate(horizon_, 1);
}
...
void MPC::UpdateLowerBound() {
  ...
  lower_bound_.head(num_states_) << -current_state_.ToVector(),
      -model_.c().replicate(horizon_, 1);
  lower_bound_.block(num_states_, 0, (horizon_ + 1) * 2, 1) =
      gap_con.replicate(horizon_ + 1, 1);
}

void MPC::UpdateUpperBound() {
  upper_bound_.head(num_states_) << -current_state_.ToVector(),
      -model_.c().replicate(horizon_, 1);
}

void MPC::SparseBlockInit(Eigen::SparseMatrix<double> &modify,
                          const Eigen::MatrixXd &block, int row_start,
                          int col_start) {
...
void MPC::SparseBlockSet(Eigen::SparseMatrix<double> &modify,
                         const Eigen::MatrixXd &block, int row_start,
                         int col_start) {
  int row_size = block.rows();
  int col_size = block.cols();
...
void MPC::SparseBlockEye(Eigen::SparseMatrix<double> &modify, int size,
                         int row_start, int col_start, int number) {
...
void MPC::DrawCar(State &state, Input &input) {
  ...
  point.x = state.x() + L * cos(state.ori()) -
            0.5 * wheel_size * cos(state.ori() + input.steer_ang());
  point.y = state.y() + L * sin(state.ori()) -
            0.5 * wheel_size * sin(state.ori() + input.steer_ang());
  point.z = 0.125;
  points_.push_back(point);
  ...
  points_.push_back(point);
  point.x = state.x() + L * 0.5 * cos(state.ori()) +
            L * 0.5 * (input.v() / constraints_.u_max()(0)) * cos(state.ori());
  point.y = state.y() + L * 0.5 * sin(state.ori()) +
            L * 0.5 * (input.v() / constraints_.u_max()(0)) * sin(state.ori());
  point.z = 0.15;
  points_.push_back(point);
