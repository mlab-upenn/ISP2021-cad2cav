...
MPC::MPC(ros::NodeHandle &nh) : input_size_(2),
                                state_size_(3),
                                constraints_(nh),
                                nh_(nh) {
    ...
    num_constraints_ = (num_states_ + 2 * (horizon_ + 1) + num_inputs_ + 2 * horizon_);
...
void MPC::Visualize() {
    ...
    // mpc_pub_.publish(Visualizer::GenerateList(points_, colors_, visualization_msgs::Marker::LINE_LIST,
    //                                           0.05, 0.0, 0.0));
...
void MPC::CreateGradientVector() {
    for (int ii = 0; ii < horizon_; ++ii) {
        gradient_.block(ii * state_size_, 0, state_size_, 1) = -1 * cost_.q() *
                                                               desired_state_trajectory_[ii].ToVector();
        gradient_.block(num_states_ + ii * input_size_, 0, input_size_, 1) = -1 * cost_.r() *
                                                                             desired_input_.ToVector();
    }
    gradient_.block(horizon_ * state_size_, 0, 3, 1) = -1 * cost_.q() *
                                                       desired_state_trajectory_[horizon_ - 1].ToVector();
}
...
void MPC::CreateLinearConstraintMatrix() {
    ...
    for (int ii = 1; ii < horizon_ + 1; ++ii) {
        SparseBlockInit(linear_matrix_, Eigen::MatrixXd::Identity(state_size_, state_size_),
                        ii * state_size_, (ii - 1) * state_size_);
        SparseBlockInit(linear_matrix_, Eigen::MatrixXd::Identity(state_size_, input_size_),
...
void MPC::SparseBlockInit(Eigen::SparseMatrix<double> &modify, const Eigen::MatrixXd &block,
...
void MPC::SparseBlockSet(Eigen::SparseMatrix<double> &modify, const Eigen::MatrixXd &block,
                         int row_start, int col_start) {
    int row_size = block.rows();
    int col_size = block.cols();
    for (int row = 0; row < row_size; ++row) {
...
void MPC::SparseBlockEye(Eigen::SparseMatrix<double> &modify, int size, int row_start,
                         int col_start, int number) {
