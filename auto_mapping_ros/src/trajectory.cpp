...
vector<float> Trajectory::GetWaypointDistances(const geometry_msgs::Pose &pose,
                                               bool inFront = true) {
  geometry_msgs::TransformStamped world_to_car_msg_stamped =
      Transforms::WorldToCarTransform(pose);
  ...
  for (auto itr = waypoints_.begin(); itr != waypoints_.end(); itr++) {
    ...
    pair<float, float> transformedPoint =
        Transforms::TransformPoint(point, world_to_car_msg_stamped);
...
int Trajectory::GetClosestIdx(const geometry_msgs::Pose pose, float lookahead) {
  ...
  for (unsigned int i = 0; i < distances.size(); i++) {
    ...
    if (currentDistance >= 0 &&
        currentDistance <
            minDistance)  // && ori >= lowerAngle && ori <= upperAngle) {
    {
      argminDist  = i;
...
pair<pair<float, float>, int> Trajectory::FindClosest(
    pair<float, float> &globalPoint) {
...
bool Trajectory::IsPathCollisionFree(const geometry_msgs::Pose pose,
                                     OccGrid &occ_grid) {
  ...
  // std::cout << "Starting: " << startingIdx << "\tEnding: " << endingIdx <<
  // std::endl;
  for (int i = startingIdx; i < endingIdx; i++) {
    int idx1 = i % waypoints_.size();
    int idx2 = (i + 1) % waypoints_.size();
    if (!occ_grid.CheckCollision(waypoints_[idx1].GetPair(),
                                 waypoints_[idx2].GetPair())) {
      return false;
    }
  }
  return true;
