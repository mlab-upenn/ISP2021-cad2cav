...
RRT::RRT(ros::NodeHandle &nh, OccGrid occ_grid)
    : nh_(nh), occ_grid_(occ_grid), gen((std::random_device())()) {
  ...
  y_dist =
      uniform_real_distribution<double>(-max_goal_distance, max_goal_distance);
  ...
  shortcut_pub =
      nh_.advertise<visualization_msgs::Marker>("shortcut_marker", 1);
...
vector<pair<float, float>> RRT::shortcutPath(vector<pair<float, float>> path) {
  if (path.size() > 2) {
    ...
    for (int i = 0; i < shortcut_iters; i++) {
      ...
      start_shortcut.first = (1 - start_t) * path[startIdx].first +
                             start_t * path[startIdx + 1].first;
      start_shortcut.second = (1 - start_t) * path[startIdx].second +
                              start_t * path[startIdx + 1].second;

      end_shortcut.first =
          (1 - end_t) * path[endIdx].first + end_t * path[endIdx + 1].first;
      end_shortcut.second =
          (1 - end_t) * path[endIdx].second + end_t * path[endIdx + 1].second;

      vector<pair<float, float>> tempPath(path.begin(),
                                          path.begin() + startIdx + 1);
...
void RRT::updateRRT(geometry_msgs::Pose &pose_update, OccGrid &occ_grid,
                    std::pair<float, float> carFrameWaypoint,
                    std::pair<float, float> globalFrameWaypoint) {
  ...
  visualization_msgs::Marker shortcut_marker =
      gen_path_marker(rrtPath, 0.5, 0, 0.5);
...
int RRT::build_tree(std::pair<float, float> targetWaypoint,
                    std::pair<float, float> targetWaypointGlobalCoords) {
  float current_angle =
      atan2(2 * current_pose_.orientation.w * current_pose_.orientation.z,
            1 - 2 * current_pose_.orientation.z * current_pose_.orientation.z);
  tree.clear();
  ...
  publish_marker(targetWaypointGlobalCoords.first,
                 targetWaypointGlobalCoords.second);
  ...
  for (int i = 0; i < rrt_iters; i++) {
    ...
    // if (check_collision(tree[x_nearest_idx], x_new) &&
    // abs(angle)<steer_angle)
    // {
    ...
    // } else
    {
      ...
      if (check_collision(tree[x_nearest_idx], x_new) &&
          angle_cost(tree, tree[x_nearest_idx], x_new) < angle_limit) {
        std::vector<int> nearby = near(tree, x_new);

        int x_min    = x_nearest_idx;
        double c_min = cost(tree, tree[x_nearest_idx]) +
                       line_cost(tree[x_nearest_idx], x_new);
        for (int current_near = 0; current_near < nearby.size();
             ++current_near) {
          if (check_collision(tree[nearby[current_near]], x_new) &&
              (cost(tree, tree[nearby[current_near]]) +
                   line_cost(x_new, tree[nearby[current_near]]) <
               c_min)) {
            x_min = nearby[current_near];
            c_min = cost(tree, tree[nearby[current_near]]) +
                    line_cost(x_new, tree[nearby[current_near]]);
          }
        }
        x_new.parent = x_min;
        ...
        if (is_goal(x_new, targetWaypointGlobalCoords.first,
                    targetWaypointGlobalCoords.second)) {
  ...
  for (auto itr = tree.begin(); itr != tree.end(); itr++) {
    ...
    geometry_msgs::TransformStamped transform_msg =
        Transforms::WorldToCarTransform(current_pose_);
    std::pair<float, float> transformedPoint =
        Transforms::TransformPoint(point, transform_msg);
...
std::vector<double> RRT::sample() {
  ...
  pair<float, float> newPoint =
      Transforms::CarPointToWorldPoint(x, y, current_pose_);
...
int RRT::nearest(std::vector<Node> &tree, std::vector<double> &sampled_point) {
  ...
  for (int i = 0; i < tree.size(); i++) {
    curr_dist = pow((tree[i]).x - sampled_point[0], 2) +
                pow((tree[i]).y - sampled_point[1], 2);
...
Node RRT::steer(Node &nearest_node, std::vector<double> &sampled_point) {
  double curr_distance = sqrt(pow(nearest_node.x - sampled_point[0], 2) +
                              pow(nearest_node.y - sampled_point[1], 2));
  Node new_node;
  vector<double> new_point;
  if (curr_distance > max_expansion_dist) {
    new_point.push_back(nearest_node.x + ((sampled_point[0] - nearest_node.x) *
                                          max_expansion_dist / curr_distance));
    new_point.push_back(nearest_node.y + ((sampled_point[1] - nearest_node.y) *
                                          max_expansion_dist / curr_distance));
  } else {
  ...
  if (abs(angle) > steer_angle && nearest_node.is_root == false) {
    ...
    double prev_angle =
        atan2(nearest_node.y - parent.y, nearest_node.x - parent.x);
...
bool RRT::is_goal(Node &latest_added_node, double goal_x, double goal_y) {
  ...
  float curr_dist   = sqrt(pow(latest_added_node.x - goal_x, 2) +
...
std::vector<pair<float, float>> RRT::find_path(std::vector<Node> &tree,
...
double RRT::angle_cost(Node &node, vector<double> &p) {
  if (!node.is_root) {
    ...
    float angle =
        (atan2(vec2.second, vec2.first) - atan2(vec1.second, vec1.first));
    return angle;
  }
}
...
double RRT::angle_cost(std::vector<Node> &tree, Node &node, Node &p) {
  if (!node.is_root) {
    ...
    vec2.first    = node.x - par_node.x;
    vec2.second   = node.y - par_node.y;
    float angle   = acos((vec1.first * vec2.first + vec1.second * vec2.second) /
                       (sqrt(pow(vec1.first, 2) + pow(vec1.second, 2)) +
                        sqrt(pow(vec2.first, 2) + pow(vec2.second, 2))));
    return angle;
  }
  return 0;
}
...
visualization_msgs::Marker RRT::gen_path_marker(
    const vector<pair<float, float>> &path, float r, float g, float b) {
...
vector<geometry_msgs::Point> RRT::in_order_path(
    const vector<pair<float, float>> &path) {
...
visualization_msgs::Marker RRT::gen_tree_marker(const vector<Node> &tree,
...
visualization_msgs::MarkerArray RRT::gen_markers(
    const vector<pair<float, float>> &points, float r, float g, float b) {
