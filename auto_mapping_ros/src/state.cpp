...
State::State(double x, double y, double ori)
    : x_(x), y_(y), ori_(ori), size_(3) {
  // ROS_INFO("state created");
}
...
// Mutator
void State::set_x(double x) { x_ = x; }

void State::set_y(double y) { y_ = y; }

void State::set_ori(double ori) { ori_ = ori; }
...
// Accessor
double State::x() { return x_; }

double State::y() { return y_; }

double State::ori() { return ori_; }

int State::size() { return size_; }