...
LocalPlanner::LocalPlanner()
  ...
  csv_relative_filepath =
      csv_relative_filepath + "_" + std::to_string(local_planner_id_) + ".csv";
  ...
  ROS_WARN_STREAM("Local planner " << local_planner_id_
                                   << " mode: " << local_planning_mode_);

  pose_sub_  = node_handle_->subscribe(pose_topic, 5,
                                      &LocalPlanner::pose_callback, this);
  scan_sub_  = node_handle_->subscribe(scan_topic, 5,
                                      &LocalPlanner::scan_callback, this);
  drive_pub_ = node_handle_->advertise<ackermann_msgs::AckermannDriveStamped>(
      drive_topic, 1);
  brake_pub_ = node_handle_->advertise<std_msgs::Bool>(brake_topic, 1);
  plan_pub_ =
      node_handle_->advertise<visualization_msgs::Marker>(plan_topic, 1);
  cov_seq_pub_ =
      node_handle_->advertise<visualization_msgs::Marker>(seq_topic, 1);

  node_handle_->getParam("lookahead_distance", lookahead_distance_);

  // Get ROS Map
  auto input_map = ros::topic::waitForMessage<nav_msgs::OccupancyGrid>(
      "map", ros::Duration(0.1));
  while (input_map == nullptr || input_map->data.empty()) {
    ROS_WARN("Waiting for Map Message on topic: %s", "map");
    input_map = ros::topic::waitForMessage<nav_msgs::OccupancyGrid>(
        "map", ros::Duration(1));
  }
  ...
  std::string csv_localtraj_path =
      ros::package::getPath(package_name) + csv_localtraj_filepath;
  ...
  std::thread global_planning_thread(&GlobalPlanner::start_global_planner,
                                     &global_planner_);
...
void LocalPlanner::pose_callback(
  ...
  State current_state(current_pose_.position.x, current_pose_.position.y,
                      current_angle);
...
void LocalPlanner::scan_callback(
...
std::vector<PlannerNode> LocalPlanner::transform(
    const std::vector<PlannerNode>& reference_way_points,
    const PlannerNode& current_way_point) {
  geometry_msgs::TransformStamped map_to_base_link;
  map_to_base_link =
      tf_buffer_.lookupTransform(base_frame_, map_frame_, ros::Time(0));
  ...
  for (const auto& reference_way_point : reference_way_points) {
    ...
    transformed_way_points.emplace_back(
        PlannerNode{map_way_point.position.x, map_way_point.position.y});
  }
...
int LocalPlanner::get_best_track_point(
    const std::vector<PlannerNode>& way_point_data) {
  ...
  for (const auto& way_point : way_point_data) {
    ...
    double distance =
        sqrt(way_point[0] * way_point[0] + way_point[1] * way_point[1]);
  ...
  /*********************************************************************************/
  /* Execute the reverse path is another good option... Save it here for later
   * use */
  ...
  ROS_DEBUG("closest_way_point: %f, %f", static_cast<double>(best_node[0]),
            static_cast<double>(best_node[1]));
  return best_index;
}

void LocalPlanner::run_pure_pursuit(
    const std::vector<PlannerNode>& reference_way_points,
    const PlannerNode& current_way_point) {
  const auto transformed_way_points =
      transform(reference_way_points, current_way_point);
  const auto goal_way_point_index =
      get_best_track_point(transformed_way_points);

  if (goal_way_point_index != -1) {
    const auto goal_way_point = transformed_way_points[goal_way_point_index];
    const auto steering_angle =
        2 * (goal_way_point[1]) / (lookahead_distance_ * lookahead_distance_);
    ...
    drive_msg.drive.steering_angle =
        steering_angle > 0.4
            ? steering_angle
            : ((steering_angle < -0.4) ? -0.4 : steering_angle);
...
void LocalPlanner::run_mpc(const std::vector<PlannerNode>& reference_way_points,
  ...
  if (goal_way_point_index != -1) {
    ...
    std::pair<float, float> goal_to_track((float)goal_way_point[0],
                                          (float)goal_way_point[1]);
...
void LocalPlanner::VisualizePlan() {
  ...
  std::vector<geometry_msgs::Point> best_traj_points =
      Visualizer::GenerateVizPoints(best_traj);
  std::vector<std_msgs::ColorRGBA> best_traj_colors =
      Visualizer::GenerateVizColors(best_traj, 0, 1, 0);
  plan_pub_.publish(
      Visualizer::GenerateList(best_traj_points, best_traj_colors));
  // visualizeCmaes();
}
...
void LocalPlanner::VisualizeCoverageSeq() {
  ...
  auto list_CovSeq_colors =
      Visualizer::GenerateVizColors(list_coverage_seq, 0, 0, 1);
  cov_seq_pub_.publish(
      Visualizer::GenerateList(list_CovSeq_points, list_CovSeq_colors,
