...
std::vector<PlannerNode> GlobalPlanner::init(
    const std::vector<PlannerNode>& sequence, double distance_threshold) {
  sequence_.clear();
  sequence_           = sequence;
...
void GlobalPlanner::update_current_position(
    const PlannerNode& current_position) {
...
void GlobalPlanner::start_global_planner() {
  ...
  while (ros::ok()) {
    ...
    const auto current_distance =
        distance(updated_position, sequence_[current_tracking_node_index_]);
    if (current_distance < distance_threshold_ || first_plan_) {
      ...
      if (current_tracking_node_index_ == sequence_.size() - 1) {
        ...
        ROS_INFO(
            "Global Planner Client is now shutting down as the sequence has "
            "been explored.");
      }
    }
  }
...
std::vector<PlannerNode> GlobalPlanner::get_next_plan(
    const PlannerNode& current_position) {
  if (sequence_.empty()) {
    ROS_ERROR(
        "The coverage sequence is empty. It needs to be set before calling "
        "get_next_plan.");
  }
...
void GlobalPlanner::update_start() {
  start_ = end_;
  ROS_INFO("Start Position: %f, %f", start_.pose.position.x,
           start_.pose.position.y);
}

void GlobalPlanner::update_end() {
  ROS_INFO("End Position: %f, %f", sequence_[current_goal_index_][0],
           sequence_[current_goal_index_][1]);
