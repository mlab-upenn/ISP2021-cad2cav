...
OccGrid::~OccGrid() { ROS_INFO("killing the occgrid"); }
...
std::pair<float, float> OccGrid::OccupancyToWorld(
...
void OccGrid::FillOccGrid(const geometry_msgs::Pose &current_pose,
                          const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
  grid_ = Eigen::MatrixXf::Zero(grid_blocks_, grid_blocks_);
  float current_angle =
      atan2(2 * current_pose.orientation.w * current_pose.orientation.z,
            1 - 2 * current_pose.orientation.z * current_pose.orientation.z);
  ...
  for (int ii = 0; ii < num_scans; ++ii) {
    float angle =
        scan_msg->angle_min + ii * scan_msg->angle_increment + current_angle;
    std::pair<float, float> cartesian =
        PolarToCartesian(scan_msg->ranges[ii], angle);
    ...
    for (float x_off = -dilation_; x_off <= dilation_; x_off += discrete_) {
      for (float y_off = -dilation_; y_off <= dilation_; y_off += discrete_) {
        std::pair<int, int> grid_point =
            WorldToOccupancy(cartesian.first + x_off, cartesian.second + y_off);
...
bool OccGrid::InGrid(int col, int row) {
  if (col >= grid_blocks_ || col < 0 || row >= grid_blocks_ || row < 0) {
    return false;
  } else {
    return true;
  }
...
bool OccGrid::CheckCollision(float x1, float y1, float x2, float y2) {
  return CheckCollision(std::pair<float, float>(x1, y1),
                        std::pair<float, float>(x2, y2));
}

bool OccGrid::CheckCollision(std::pair<float, float> first_point,
                             std::pair<float, float> second_point) {
  ...
  if (!(InGrid(first_point_grid) && InGrid(second_point_grid))) {
