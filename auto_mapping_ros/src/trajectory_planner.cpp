...
TrajectoryPlanner::TrajectoryPlanner(ros::NodeHandle &nh)
  ...
  std::string traj_pub_topic = std::string("trajectory_planner_") +
                               std::to_string(trajectory_planner_id_);
...
void TrajectoryPlanner::ReadTrajectories(string path) {
  ...
  if (input.is_open()) {
    ...
    // each trajectory has 10 pairs of points, total 100 pairs are present for
    // 10 trajectories
  } else {
...
pair<float, float> TrajectoryPlanner::CarPoint2World(
    float x, float y, const geometry_msgs::Pose &current_pose) {
...
void TrajectoryPlanner::Trajectory2world(
    const geometry_msgs::Pose &current_pose) {
  trajectories_world.clear();
  for (unsigned int i = 0; i < trajectories_.size(); i++) {
    trajectories_world.push_back(CarPoint2World(
        trajectories_[i].first, trajectories_[i].second, current_pose));
  }
  // ROS_INFO("trajectories in world frame");
}

int TrajectoryPlanner::BestTrajectory(OccGrid &occ_grid,
                                      const geometry_msgs::Pose &current_pose,
                                      std::pair<float, float> &goal_to_track) {
  ...
  for (int ii = 0; ii < num_traj_ / 2; ii++) {
    ...
    for (int l = 0; l < horizon_ - 1; l++) {
      collision = occ_grid.CheckCollision(
          trajectories_world[max_horizon_ * ii + l],
    ...
    if (collision) {
      ...
      pair<float, float> end_point =
          trajectories_world[max_horizon_ * ii + horizon_ - 1];
  ...
  best_cmaes_point_.set_x(
      trajectories_world[max_horizon_ * best + horizon_ - 1].first);
  best_cmaes_point_.set_y(
      trajectories_world[max_horizon_ * best + horizon_ - 1].second);
...
void TrajectoryPlanner::SelectLane(const geometry_msgs::Pose pose,
                                   OccGrid &occ_grid) {
  unsigned int old_lane = current_lane_;
  for (unsigned int lane = 0; lane < lanes_.size(); ++lane) {
    if (lane != current_lane_ &&
        lanes_[lane].IsPathCollisionFree(pose, occ_grid)) {
      current_lane_ = lane;
...
vector<State> TrajectoryPlanner::best_minipath() { return best_minipath_; }
...
void TrajectoryPlanner::Visualize() {
  ...
  for (int i = max_horizon_ * best_trajectory_index_;
       i < max_horizon_ * best_trajectory_index_ + horizon_; i++) {
  ...
  std::vector<std_msgs::ColorRGBA> best_traj_colors =
      Visualizer::GenerateVizColors(best_traj, 1, 1, 0);
  points_.insert(points_.end(), best_traj_points.begin(),
                 best_traj_points.end());
  colors_.insert(colors_.end(), best_traj_colors.begin(),
                 best_traj_colors.end());
  ...
  std::vector<geometry_msgs::Point> cmaes_points =
      Visualizer::GenerateVizPoints(pts);
  std::vector<std_msgs::ColorRGBA> cmaes_colors =
      Visualizer::GenerateVizColors(pts, 1, 0, 0);
...
void TrajectoryPlanner::Update(const geometry_msgs::Pose &current_pose,
                               OccGrid &occ_grid,
                               std::pair<float, float> &goal_to_track) {
  // trajectory2miniworld(current_pose);
  // distance_from_switch_ += Transforms::CalcDist(std::pair<float,
  // float>(last_pose_.position.x, last_pose_.position.y),
  //                                               std::pair<float,
  //                                               float>(current_pose.position.x,
  //                                               current_pose.position.y));
  Trajectory2world(current_pose);

  // if (distance_from_switch_ > switch_distance_threshold_ ||
  //     !lanes_[current_lane_].IsPathCollisionFree(current_pose, occ_grid)) {
  //     SelectLane(current_pose, occ_grid);
  // }

  best_trajectory_index_ =
      BestTrajectory(occ_grid, current_pose, goal_to_track);
