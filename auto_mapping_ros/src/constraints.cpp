...
Constraints::Constraints(ros::NodeHandle &nh) {
  ...
  slip_upper_bound_ << slip_p1_steer - slope * slip_p1_vel,
      slip_p1_steer - slope * slip_p1_vel;
  slip_lower_bound_.resize(2, 1);
  slip_lower_bound_ << -slip_p1_steer + slope * slip_p1_vel,
      -slip_p1_steer + slope * slip_p1_vel;
  points_pub_ =
      nh.advertise<visualization_msgs::Marker>("triangle_points", 100);
}

Constraints::~Constraints() {}

Eigen::VectorXd Constraints::x_max() { return x_max_; }

Eigen::VectorXd Constraints::x_min() { return x_min_; }

Eigen::VectorXd Constraints::u_max() { return u_max_; }

Eigen::VectorXd Constraints::u_min() { return u_min_; }

Eigen::MatrixXd Constraints::slip_constraint() { return slip_constraint_; }
Eigen::MatrixXd Constraints::slip_upper_bound() { return slip_upper_bound_; }

Eigen::MatrixXd Constraints::slip_lower_bound() { return slip_lower_bound_; }
...
Eigen::VectorXd Constraints::l1() { return l1_; }

Eigen::VectorXd Constraints::l2() { return l2_; }
...
void Constraints::set_state(State &state) { state_ = state; }
...
void Constraints::FindHalfSpaces(State &state,
                                 sensor_msgs::LaserScan &scan_msg_) {
  int num_scans =
      (scan_msg_.angle_max - scan_msg_.angle_min) / scan_msg_.angle_increment +
      1;
  int max_gap         = -1;
  int best_lo         = 0;
  int best_hi         = 0;
  ...
  for (int ii = 0; ii < num_scans; ii++) {
    ...
    if (angle > -1.571f / divider_ &&
        angle < 1.571f / divider_)  // value is pi/2
    {
  ...
  float angle1 =
      scan_msg_.angle_min + best_lo * scan_msg_.angle_increment + current_angle;
  float angle2 =
      scan_msg_.angle_min + best_hi * scan_msg_.angle_increment + current_angle;

  p1_.first = scan_msg_.ranges[best_lo] * cos(angle1) +
              poseX;  // - 0.275 * cos(current_angle);
  p1_.second = scan_msg_.ranges[best_lo] * sin(angle1) +
               poseY;  // - 0.275 * sin(current_angle);

  p2_.first = scan_msg_.ranges[best_hi] * cos(angle2) +
              poseX;  //- 0.275 * cos(current_angle);
  p2_.second = scan_msg_.ranges[best_hi] * sin(angle2) +
               poseY;  //- 0.275 * sin(current_angle);
