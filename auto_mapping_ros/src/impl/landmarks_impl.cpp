...
std::vector<Frontier> FrontierFinder::find_frontiers(
    const std::array<int, 2>& point, const cv::Mat& map) const {
  ...
  cv::Mat visited =
      cv::Mat(map.rows, map.cols, cv::DataType<bool>::type, cv::Scalar(false));
  ...
  for (int i = 0; i < frontier_map.rows; i++) {
    for (int j = 0; j < frontier_map.cols; j++) {
      if (frontier_map.at<bool>(i, j) &&
          !static_cast<bool>(visited.at<uchar>(i, j))) {
        Frontier frontier{};
        run_dfs_and_update_frontier_groups(i, j, frontier_map, &visited,
                                           &frontier.frontier);
...
int FrontierFinder::find_n_frontiers(const std::array<int, 2>& point,
                                     const cv::Mat& map) const {
  return static_cast<int>(find_frontiers(point, map).size());
}

/// Runs DFS on the point (i, j) which is a frontier cell and update the
/// frontier group vector with all the points belonging to the frontier group
/// connected to the point
/// @param row_index
/// @param col_index
/// @param frontier_map
/// @param frontier_group
void FrontierFinder::run_dfs_and_update_frontier_groups(
    int row_index, int col_index, const cv::Mat& frontier_map, cv::Mat* visited,
  ...
  for (unsigned int i = 0; i < neighbor_indices.size(); i++) {
    ...
    if (!is_valid_cell(neighbor_point_x, neighbor_point_y, frontier_map.rows,
                       frontier_map.cols) ||
        !frontier_map.at<bool>(neighbor_point_x, neighbor_point_y))
      continue;

    run_dfs_and_update_frontier_groups(neighbor_point_x, neighbor_point_y,
                                       frontier_map, visited, frontier_group);
  }
  return;
}
...
/// @param map
/// @return
bool FrontierFinder::is_frontier_cell(int row_index, int col_index,
                                      const cv::Mat& map) const {
  for (unsigned int i = 0; i < neighbor_indices.size(); i++) {
    int neighbor_row_index = row_index + neighbor_indices[i][0];
    int neighbor_col_index = col_index + neighbor_indices[i][1];
    if (is_valid_cell(row_index + neighbor_indices[i][0],
                      col_index + neighbor_indices[i][1], map.rows, map.cols)) {
      if (static_cast<int>(
              map.at<uchar>(neighbor_row_index, neighbor_col_index)) ==
          frontier_config.unknown_value) {
        return true;
      }
    }
  }
...
/// @return
bool FrontierFinder::is_valid_cell(int row_index, int col_index, int map_rows,
                                   int map_cols) const {
  if (row_index < 0 || row_index > map_rows - 1 || col_index < 0 ||
      col_index > map_cols - 1) {
    return false;
  }
  return true;
}

std::vector<Frontier> FrontierFinder::remove_minor_frontiers(
...
cv::Mat FrontierFinder::ray_cast_to_2d_map(const std::array<int, 2>& point,
                                           const cv::Mat& map) const {
  cv::Mat ray_casted_map = cv::Mat(map.rows, map.cols, CV_8UC1,
                                   cv::Scalar(frontier_config.unknown_value));
  const auto angle_increment =
      ray_casting_config.fov / ray_casting_config.n_rays;
...
/// Construct a 2D map around the point and returns an image of the size
/// constant value more than maximum range of raycasting
/// @param point
/// @param map
/// @return
cv::Mat FrontierFinder::ray_cast_to_2d_sub_map(const std::array<int, 2>& point,
                                               const cv::Mat& map) const {
  const auto map_size = static_cast<int>(ray_casting_config.max_valid_range /
                                         ray_casting_config.grid_size) +
                        50;
...
double distance_temp(const std::array<double, 2>& node1,
                     const std::array<double, 2>& node2) {
  return sqrt(pow(node1[0] - node2[0], 2) + pow(node1[1] - node2[1], 2));
}

/// Casts a single ray (point, current_angle) and updates the ray_cast_map based
/// on the hits in map
/// @param point
/// @param current_angle
/// @param map
...
void FrontierFinder::single_ray_cast(const std::array<int, 2>& point,
  ...
  double start_x_m = point[0] * ray_casting_config.grid_size +
                     ray_casting_config.grid_size / 2;
  double start_y_m = point[1] * ray_casting_config.grid_size +
                     ray_casting_config.grid_size / 2;
  ...
  while (current_x_m >= 0 && current_y_m >= 0 &&
         ...
         distance_temp(std::array<double, 2>{start_x_m, start_y_m},
                       std::array<double, 2>{current_x_m, current_y_m}) <
             ray_casting_config.max_valid_range) {
    const auto [x, y] = get_tile_coords({current_x_m, current_y_m});
    if (static_cast<int>(map.at<uchar>(x, y)) <=
        static_cast<int>(frontier_config.max_occupied_threshold)) {
...
/// @param point
/// @return
std::array<int, 2> FrontierFinder::get_tile_coords(
    const std::array<double, 2>& point) const {
  return std::array<int, 2>{
      static_cast<int>(floor(point[0] / ray_casting_config.grid_size)),
...
/// @param current_angle
/// @return
std::array<double, 2> FrontierFinder::get_xy_direction_components(
    const double current_angle) const {
