...
void print_vector_of_nodes(
    const std::vector<std::array<double, 2>>& vector_of_nodes) {
...
void print_graph(const Graph& graph) {
  ...
  for (const auto& node : graph) {
    ...
    for (unsigned int i = 0; i < node.neighbors.size(); i++) {
      std::cout << "\t{" << node.neighbors[i]->id << ", "
                << node.neighbors_cost[i] << ", (x: " << node.neighbors[i]->x
                << ", y: " << node.neighbors[i][0].y << ") }";
    }
    std::cout << "\n";
  }
}
...
void get_clicked_point(int event, int x, int y, int flags, void* userdata) {
  if (event == cv::EVENT_LBUTTONDOWN) {
    ...
    ROS_INFO_STREAM("Registered click at: (" << clicked_point[0] << ", "
                                             << clicked_point[1] << ")");
    clicked = true;
  }
}
...
int get_closest_clicked_node_on_map(const cv::Mat& map, aco::Graph& graph) {
  ...
  for (const auto& node : graph) {
    cv::circle(visual_graph,
               {static_cast<int>(node.y), static_cast<int>(node.x)}, 4,
               cv::Scalar(100, 0, 0));
  }

  const std::string window_name =
      "Initial Depot Registration - Please click on the window";
  ...
  for (const auto& node : graph) {
    double distance = sqrt(pow(node.x - clicked_point[0], 2) +
                           pow(node.y - clicked_point[1], 2));
