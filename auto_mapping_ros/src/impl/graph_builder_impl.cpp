...
void GraphBuilder::init_config() {
  ...
  try {
    const std::string filename =
        get_package_directory() + "/config/graph_builder.cfg";
...
/// \param flags
/// \param userdata
void GraphBuilder::CallBackFunc(int event, int x, int y, int flags,
                                void *userdata) {
...
void GraphBuilder::register_boundary() {
  const std::string window_name =
      "Functional Area Registration - Please click on the window";
...
std::vector<std::array<int, 2>> GraphBuilder::compute_blob_centers(
...
/// Filters the corners vector by finding sparse matrices using morphological
/// techniques
...
/// Finds all the important features/corners in the blueprint which can be used
/// as nodes
...
std::vector<std::array<int, 2>> GraphBuilder::find_corners() const {
  ...
  for (int i = 0; i < dst_norm.rows; i++) {
    for (int j = 0; j < dst_norm.cols; j++) {
      if (static_cast<int>(dst_norm.at<float>(i, j)) > 100 &&
          is_within_specified_region(j, i)) {
  ...
  std::set<std::array<int, 2>> sparse_centroid_set(
      dense_corner_centroids.begin(), dense_corner_centroids.end());
  unique_sparse_centroids.assign(sparse_centroid_set.begin(),
                                 sparse_centroid_set.end());

  ROS_INFO_STREAM("There are " << unique_sparse_centroids.size()
...
bool GraphBuilder::check_collision(const Node &current_node,
                                   const Node &neighbor_node) const {
  ...
  for (int v = 0; v < steps; v++) {
    ...
    const auto pixel_value = static_cast<int>(map_.at<uchar>(
        static_cast<int>(intermediate_x), static_cast<int>(intermediate_y)));
...
/// @param corners
void GraphBuilder::construct_graph(
  ...
  for (auto &node : graph_) {
    ...
    for (const auto &neighbor_node : node.neighbors) {
      const auto distance =
          static_cast<double>(sqrt(pow(node.y - neighbor_node->y, 2) +
                                   pow(node.x - neighbor_node->x, 2)));
...
void GraphBuilder::trim_edges() {
  for (auto &current_node : graph_) {
    const unsigned int current_node_neighbors_size =
        current_node.neighbors.size();
...
/// Checks if any neighbors of the current node lie between the current node and
/// the neighbor node
/// @param current_node - current node of the graph
/// @param neighbor_node - neighbor node of the current node of the graph
/// @return true if a neighbor of current node lies within a threshold distance
/// of the two input nodes
bool GraphBuilder::is_another_node_in_between(Node *current_node,
                                              Node *neighbor_node) const {
  ...
  for (const auto &current_node_neighbor : current_node->neighbors) {
    ...
    const auto dist = pDistance(
        current_node_neighbor->x, current_node_neighbor->y, current_node->x,
        current_node->y, neighbor_node->x, neighbor_node->y);
    if (dist < threshold) {
      return true;
    }
  }
...
double GraphBuilder::pDistance(double x_point, double y_point,
                               double x_line_segment_start,
                               double y_line_segment_start,
                               double x_line_segment_end,
                               double y_line_segment_end) const {
...
void GraphBuilder::run_dfs(Node *node, std::unordered_set<int> &visited_nodes,
...
void GraphBuilder::graph_subset(std::vector<Node> &subset_nodes) {
  ...
  for (unsigned int i = 0; i < subset_nodes.size(); i++) {
    ...
    if (!main_graph_current_node)
      throw std::runtime_error("empty node ptr in GraphBuilder");
...
Graph GraphBuilder::find_largest_connected_component() {
  ...
  for (auto &node : graph_) {
    ...
    if (current_connected_component.size() >
        largest_connected_component.size()) {
