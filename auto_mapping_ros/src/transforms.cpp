...
std::pair<float, float> Transforms::CarPointToWorldPoint(
    float x, float y, geometry_msgs::Pose &current_pose) {
  ...
  return std::pair<float, float>(worldPoint.x + carPoseX,
                                 worldPoint.y + carPoseY);
}

geometry_msgs::TransformStamped Transforms::WorldToCarTransform(
    const geometry_msgs::Pose &pose) {
  tf2::Transform car_to_world;
  tf2::fromMsg(pose, car_to_world);
  tf2::Transform world_to_car = car_to_world.inverse();
  geometry_msgs::Transform world_to_car_msg =
      tf2::toMsg(world_to_car);  // H^c_w
...
std::pair<float, float> Transforms::TransformPoint(
...
float Transforms::GetCarOrientation(geometry_msgs::Pose pose) {
  return atan2(2 * pose.orientation.w * pose.orientation.z,
               1 - 2 * pose.orientation.z * pose.orientation.z);
}

float Transforms::CalcDist(std::pair<float, float> p1,
