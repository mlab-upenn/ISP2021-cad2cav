...
std::array<double, 2> translate_indices_to_xy(
    const std::array<int, 2>& node_on_previous_map, double resolution);
...
double distance(const std::array<Arithmetic, 2>& node1,
                const std::array<Arithmetic, 2>& node2);
...
template <typename T>
void write_sequence_to_csv(const std::vector<std::array<T, 2>>& sequence,
                           std::string filename, bool switch_xy = false) {
