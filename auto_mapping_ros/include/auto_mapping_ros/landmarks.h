...
struct Frontier {
  ...
  /// Removes minor frontiers which have frontier cells lesser than
  /// 'min_frontier_cells'
...
public:
  ...
  std::vector<Frontier> find_frontiers(const std::array<int, 2>& point,
  ...
  /// Construct a 2D map around the point and returns an image of the size of
  /// map
  /// @param point
  /// @param map
  /// @param config
  ...
  /// @param map
  /// @return
  cv::Mat ray_cast_to_2d_sub_map(const std::array<int, 2>& point,
                                 const cv::Mat& map) const;

  std::vector<Frontier> remove_minor_frontiers(
...
private:
  ...
  void run_dfs_and_update_frontier_groups(
      int row_index, int col_index, const cv::Mat& frontier_map,
      cv::Mat* visited, std::vector<std::array<int, 2>>* frontier_group) const;

  bool is_neighbor_frontier(int row_index, int col_index,
                            const cv::Mat& map) const;
  ...
  /// @return
  bool is_valid_cell(int row_index, int col_index, int map_rows,
                     int map_cols) const;

  /// Get the x y components of the ray direction (at current angle)
  /// @param current_angle
  /// @return
  std::array<double, 2> get_xy_direction_components(
  ...
  /// Casts a single ray (point, current_angle) and updates the ray_cast_map
  /// based on the hits in map
  /// @param point
  /// @param current_angle
  /// @param map
  /// @param ray_cast_map
  void single_ray_cast(const std::array<int, 2>& point,
                       const double current_angle, const cv::Mat& map,
