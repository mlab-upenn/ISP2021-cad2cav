...
public:
  ...
  /// @param skeletonized_image - image obtained from skeletonization and
  /// subtracting the blueprint
...
private:
  ...
  std::vector<std::array<int, 2>> compute_blob_centers(
  ...
  /// Finds all the important features/corners in the blueprint which can be
  /// used as nodes
  ...
  double pDistance(double x_point, double y_point, double x_line_segment_start,
                   double y_line_segment_start, double x_line_segment_end,
                   double y_line_segment_end) const;

  /// Checks if any neighbors of the current node lie between the current node
  ...
  /// @return true if a neighbor of current node lies within a threshold
  /// distance of the two input nodes
  bool is_another_node_in_between(Node* current_node,
  ...
  bool check_collision(const Node& current_node,
                       const Node& neighbor_node) const;

  void run_dfs(Node* node, std::unordered_set<int>& visited_nodes,
