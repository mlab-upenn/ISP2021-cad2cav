...
public:
  ...
  virtual ~MPC();
  // Runs one iteration of MPC given the latest state from callback and last MPC
  // input Uses desired_state_trajectory for tracking reference
  void Update(State current_state, Input input,
...
private:
  ...
  // Updates the gradient vector for costs. The gradient vector is fully updated
  // each time.
  ...
  // Updates the non-constant parts of the upper and lower bound constraint
  // vectors
  ...
  void SparseBlockSet(Eigen::SparseMatrix<double> &modify,
                      const Eigen::MatrixXd &block, int row_start,
                      int col_start);
  // Sets just the ones of an identity block in a sparse matrix
  void SparseBlockEye(Eigen::SparseMatrix<double> &modify, int size,
                      int row_start, int col_start, int number);
  // Pushes lines that create a state and input representation of the car for
  // visualization
