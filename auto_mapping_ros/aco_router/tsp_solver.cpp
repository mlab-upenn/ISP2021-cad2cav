...
void create_colony(const aco::Graph& graph,
                   ...
                   const aco::AcoParams& params, const int initial_node_id) {
    ...
    for (int i = 0; i < params.n_ants; i++) {
        ...
        // Move the ant through the n nodes based on the probabilities received
...
void update_pheromone_value(const std::vector<std::vector<aco::Node>>& colony,
    ...
    for (int ant_index = 0; ant_index < colony.size(); ant_index++) {
        for (int node_index = 0; node_index < colony[ant_index].size() - 1;
             node_index++) {
            ...
            tau(current_node_id, next_node_id) =
                tau(current_node_id, next_node_id) +
                (1 / fitness_values[ant_index]);
        }
    }
}

/**
 * Function to solve the traveling salesman problem for single salesman using
 * ant colony optimization
 * @param graph
 * @param params
 * @return
 */
std::pair<std::vector<aco::Node>, double> aco::solve_tsp(const Graph& graph,
                                                         int initial_node_id) {
    ...
    for (int i = 0; i < params.max_iters; i++) {
        ...
        const auto min_index =
            std::distance(ant_fitness_value.begin(),
                          std::min_element(ant_fitness_value.begin(),
                                           ant_fitness_value.end()));
