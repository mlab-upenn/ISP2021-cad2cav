...
std::vector<aco::Node> opt2_swap(const std::vector<aco::Node>& existing_sequence, const int i, const int k)
{
 ...
 */
void aco::run_opt2(const Eigen::MatrixXd& cost_matrix, std::vector<aco::Node>& sequence)
{
    // Get opt-2 algorithm parameters
    const auto params = get_opt2_params();
    if(!params.use_opt2) return;

    if(params.max_iters != -1) { std::cout << "Cannot set max iters yet. running opt2 till improvement stops.\n";}

    auto best_fitness_value = find_fitness_values(cost_matrix, sequence);
    std::cout << "Fitness value of sequence before Opt2: " << best_fitness_value << std::endl;
    int I = 0;
    while(true)
    {
        for(int i=1; i<sequence.size()-2; i++)
        {
            for(int k=i+1; k<sequence.size()-1; k++)
            {
                ...
                const auto new_fitness = find_fitness_values(cost_matrix, new_sequence);
    ...
    std::cout << "Fitness value of sequence after Opt2: " << best_fitness_value << std::endl;
}
